  ...
  const ingredientRows = (() => {
    if (vatScales.length <= 1) {
      return ingredientSteps.map((s) => ({
        ...
        display: s.base_quantity ? `${parseFloat((s.base_quantity * totalScaleFactor).toFixed(2))} ${s.unit}` : '',
      }))
    }
    ...
    <section style={pageBreakBefore ? { pageBreakBefore: 'always' } : undefined} className="print-recipe">
      ...
      <div>
        ...
        {(() => {
          const renderStep = (s) => (
            <li key={s.id}>
              <span className="opacity-60 text-xs">[{STEP_TYPE_LABELS[s.type]}]</span> {getStepDisplayTitle(s)}
              {s.type === 'ingredient_addition' && s.base_quantity ? ` — ${parseFloat((s.base_quantity * stepListScaleFactor).toFixed(2))} ${s.unit}` : ''}
